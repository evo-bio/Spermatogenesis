...
Human_counts %>% 
  ...
  inner_join( Human_tau ) %>% 
  gather(CELL,count,-TissueTau,-GENE) %>% 
  inner_join(Human_meta_data) %>%
  filter(count != 0) %>%
  filter(!is.na(TissueTau)) %>%
  group_by(CELL,CELL_TYPE) %>% 
  summarise( MEAN = mean(TissueTau),
             MEDIAN = median(TissueTau) ) -> Human_gathered
...
Human_gathered %>%
  ggplot() +
  geom_boxplot( aes(CELL_TYPE,MEDIAN,fill=CELL_TYPE),notch=T )+
  theme_cowplot()+
